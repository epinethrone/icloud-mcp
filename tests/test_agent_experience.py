...
def test_instructions_tell_agents_who_the_owner_is_and_how_to_invite(s):
    ...
    assert "INVITING PEOPLE" in text and "mail_search" in text and "ONE call" in text
...
def test_instructions_carry_the_mail_workflow(s):
    text = build_instructions(s)
    assert "MAIL:" in text and "mail_reply(folder, uid, body)" in text and "never silently dropped" in text
