// Mark a reminder complete (or incomplete). Arguments: id, completed (bool, default true). The helper may add an internal `hint`.
...
function run(argv) {
  ...
  var cur = props(guard(loc, a.id));                                // read everything first: completing may move the item
  var done = (a.completed !== false);
  guard(loc, a.id).completed = done;
  return JSON.stringify({ id: cur.id, title: cur.title, completed: done, position: loc.idx });
}
