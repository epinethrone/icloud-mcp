// INTERNAL (the helper's cache refresh, not an operation the server can request). Arguments: list_id.
// Returns the OPEN reminders of one list, each with its position. Completed reminders are never read: one scan finds which are open (one
// batch read of the completed flags), then only those items are read by position, which is instant.
...
function run(argv) {
  ...
  var list = app.lists.byId(a.list_id);
  var name = list.name(), account = null;
  try { account = list.container().name(); } catch (e) {}
  var rs = list.reminders;
  var done = rs.completed();
  var items = [];
  for (var j = 0; j < done.length; j++) {
    if (done[j]) continue;
    var p = props(rs[j]);
    if (p.completed) continue;                                       // changed while we were reading
    items.push({ id: p.id, title: p.title, notes: p.notes, due: p.due ? p.due.toISOString() : null, priority: p.priority, position: j });
  }
  return JSON.stringify({ list_id: a.list_id, list: name, account: account, count: done.length, items: items });
}
